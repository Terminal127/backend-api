...
# HTML endpoint to dynamically generate background images
@app.route("/background")
@login_required
def generate_background():
    # Fetch the list of images from your /images endpoint
    images_response = requests.get("http://fastapi:3000/images") # use service names for referencing
    images = images_response.json()

    if not images:
        return "<html><body>No images available</body></html>"

    # Select a random image URL for the background
    background_url = random.choice(images)["url"]

    return render_template('background.html', background_url=background_url, images=images)
